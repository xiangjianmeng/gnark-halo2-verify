...
var MODULUS *big.Int = emulated.BN254Fr{}.Modulus()

// The number of bits to use for range checks on inner products of field elements.
// This MUST be a multiple of EXPECTED_OPTIMAL_BASE_WIDTH if the commit based range checker is used.
// There is a bug in the pre 0.9.2 gnark range checker where it wouldn't appropriately range check a bitwidth that
// is misaligned from EXPECTED_OPTIMAL_BASE_WIDTH:  https://github.com/Consensys/gnark/security/advisories/GHSA-rjjm-x32p-m3f7
var RANGE_CHECK_NB_BITS int = 144

var EXPECTED_OPTIMAL_BASE_WIDTH int = 16
...
// Variable A type alias used to represent Goldilocks field elements.
type Variable struct {
	Limb frontend.Variable
}

// NewVariable Creates a new Goldilocks field element from an existing variable. Assumes that the element is
// already reduced.
func NewVariable(x frontend.Variable) Variable {
	return Variable{Limb: x}
}

// Zero The zero element in the Golidlocks field.
func Zero() Variable {
...
func InverseHint(_ *big.Int, inputs []*big.Int, results []*big.Int) error {
	...
	inputGl := fr.NewElement(input.Uint64())
	resultGl := fr.NewElement(0)
...
func (p *Chip) checkCollected(api frontend.API) error {
	...
	if nbBits != EXPECTED_OPTIMAL_BASE_WIDTH {
		panic("nbBits should be " + strconv.Itoa(EXPECTED_OPTIMAL_BASE_WIDTH))
	}
